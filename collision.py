    ...
    def collide_bounds(self):
        ...
        for i in range(self.scene.num_boxes):
            ...
            for j in range(self.scene.nboundary):
                ...
                if is1cp:
                    print("detected boundary contact")
    ...
    def collide_all(self):
        for i in range(self.scene.N):
            ...
            for j in range(i):
                ...
                if tm.distance(self.scene.objects[i].p, self.scene.objects[j].p) > R:
                    continue

                incident_body, iv, ie, sep = self.collide_box_box(self.scene.objects[i],
                ...
                if sep < 0:
                    ...
                    nColl = rot(self.scene.objects[iRef].q, self.scene.normals[ie])
                    ...
                    if sep1 <= radInc + radRef:
                        ...
                        rInc1 = xColl1 - self.scene.objects[iInc].p
                        rRef1 = xColl1 - self.scene.objects[iRef].p
                        n_pc += 1
                        is1cp = True
                    if sep2 <= radInc + radRef:
                        ...
                        rInc2 = xColl2 - self.scene.objects[iInc].p
